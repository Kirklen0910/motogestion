...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($error)) {
        $stmt = $pdo->prepare("INSERT INTO products (code, name, image, cost, price, stock, min_stock, supplier_id) VALUES (?,?,?,?,?,?,?,?)");
        $stmt->execute([$code, $name, $image, $cost, $price, $stock, $min_stock, $supplier_id]);
        
        $newId = $pdo->lastInsertId();
        
        // Registrar movimiento de entrada en inventory_movements (CORREGIDO)
        $stmt_movement = $pdo->prepare("INSERT INTO inventory_movements (product_id, type, quantity, reference, user_id) VALUES (?, 'IN', ?, 'CREACION_PRODUCTO', ?)");
        $stmt_movement->execute([$newId, $stock, $_SESSION['user_id']]);
        
        $ganancia = $price - $cost;
        registerLog($pdo, 'CREATE', 'products', $newId, "Producto creado: $name - Código: $code - Costo: L $cost - Precio: L $price - Ganancia: L $ganancia");
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" step="0.01" name="cost" class="form-control" required id="cost" onchange="calcularGanancia()">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label>Precio Venta (Lps) *</label>
                                    <input type="number" step="0.01" name="price" class="form-control" required id="price" onchange="calcularGanancia()">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="alert alert-info" id="gananciaPreview">
                                    <i class="fas fa-chart-line"></i> Ganancia por unidad: <strong>L 0.00</strong>
                                </div>
                            </div>
                        </div>
